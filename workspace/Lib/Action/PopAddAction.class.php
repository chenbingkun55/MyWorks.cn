...
class PopAddAction extends Action {
	public function view_box_conten( $pid,$rid,$uid ){
		...
		echo "<form action=\"/index.php/PopAdd/add_box_conten/\" method=\"post\" >";
		echo "资源类型: ";
		...
		$this->view_box_list( $arr_re[$rid_index]['R_type'] );
		...
		echo "<input name=\"R_type\" type=\"hidden\" value=\"".$arr_re[$rid_index]['R_type']."\"><BR>";
		echo "<input type=\"submit\" value=\"添加\">";
		echo "</form>";
		echo "</DIV>";
	}

	public function view_box_list( $rid ){
		switch( $rid ){
			case "1" :
				echo "服务器名称:<input name=\"S_name\" type=\"text\"><BR>";
				echo "服务器类型:<input name=\"S_type\" type=\"text\"><BR>";
				echo "服务器备注:<input name=\"S_note\" type=\"text\"><BR>";
				echo "IP:<input name=\"S_ip1\" type=\"text\"><BR>";
				echo "子网掩码:<input name=\"S_sub1\" type=\"text\"><BR>";
				echo "IP2:<input name=\"S_ip2\" type=\"text\"><BR>";
				echo "子网掩码2:<input name=\"S_sub2\" type=\"text\"><BR>";
				echo "<input name=\"S_env\" type=\"hidden\" value=\"23456\"><BR>";
				break;
			case "2" :
	...
	public function add_box_conten( ){
		switch( $_POST['R_type'] ){
			...
			case "3" :
					...
					$work_password->create();
					if( $work_password->add() ){
						header("Location: /");
					}
				break;
			case "4" :
	...
	public function view_leftbar_box( $lid,$pid,$rid,$uid ){
		...
		echo "<DIV id=\"pop_window_conten\" class=\"pop_window_conten\">";
		echo "<form action=\"/index.php/PopAdd/add_leftbar_box/\" method=\"post\" >";
		echo "资源类型: ";
	...
	public function view_toolbar_box( $lid,$pid,$rid,$uid ){
		if ( $lid == 0 ){
			...
			echo "<DIV id=\"pop_window_conten\" class=\"pop_window_conten\">";
			echo "<form action=\"/index.php/PopAdd/add_toolbar_box/\" method=\"post\" >";
			echo "位置: ";
		...
		switch( $_POST['toolbar'] ){
			...
			case "tool_bar_m2" :
				...
				if( ! $work_project->create()){
					...
					$work_project->add() ;
					// 跳转不到
					// header("Location: /index.php/Ajax/tool_bar/menu/tool_bar_m2/index/0/lid/".$_POST['L_id']."/pid/0/uid/1/");
					header("Location: /");
				}
				break;
			case "tool_bar_m3" :
				...
				if( ! $work_resource->create()){
					...
					$work_resource->add() ;
					// 跳转不到
					// header("Location: /index.php/Ajax/tool_bar/menu/tool_bar_m3/index/0/lid/".$_POST['L_id']."/pid/".$_POST['P_id']."/uid/1/");
					header("Location: /");
				}
				break;
			default :
